...
class Find_person(smach.State):
	...
	def execute(self, userdata):
		...
		self.target = str(Command.data[0].target)
		self.option = str(Command.data[0].option)
		...
		if find_person_flag == False:
			...
			f = find_person.processing(self.target, self.option)
...
class Ask(smach.State):
	...
	def execute(self, userdata):
		...
		if ask_flag == False:
			...
			a = ask.processing(self.target, self.option)
			Report_word = a
	...
	def execute(self, userdata):
		...
		# 命令のステートがすべて終了
			...
			if Task_times == Task_count:
				...
				#部屋から出るために１００ｃｍ前進
				get_out = str_send_wheel.processing()
